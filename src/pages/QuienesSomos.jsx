...
const STAT_META = [
  ...
  { icon: MapPin,   label: 'Municipios analizados',     color: 'bg-slate-50 text-slate-700 dark:bg-slate-700 dark:text-slate-500' },
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm font-semibold text-[#0F172A] dark:text-slate-100">{pct}%</span>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
      ...
      <m.div
        ...
        className="mb-16"
      >
        <h1 className="font-display text-4xl sm:text-5xl font-bold text-[#0F172A] dark:text-slate-100 tracking-tight mb-5 leading-tight">
        ...
        <p className="text-lg text-slate-600 dark:text-slate-500 max-w-2xl leading-relaxed">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-20">
        {stats.map((s, i) => (
          ...
          >
            ...
            <div>
              <p className="text-3xl font-bold text-[#0F172A] dark:text-slate-100 leading-none">{s.value}</p>
              <p className="text-xs text-slate-500 dark:text-slate-500 mt-1 leading-snug">{s.label}</p>
            </div>
          </m.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20 items-start">
        ...
        >
          <div className="border-b-2 border-[#0F172A] dark:border-slate-600 pb-3 mb-6">
            <h2 className="font-display text-3xl font-bold text-[#0F172A] dark:text-slate-100 tracking-tight">Nuestra misión</h2>
          </div>
          <div className="space-y-4 text-slate-600 dark:text-slate-500 leading-relaxed">
            <p>
        ...
        >
          {PILARES.map((p, i) => (
            ...
            >
              ...
              <div>
                <p className="font-semibold text-[#0F172A] dark:text-slate-100 mb-1">{p.title}</p>
                <p className="text-sm text-slate-500 dark:text-slate-500 leading-relaxed">{p.desc}</p>
              </div>
            </m.div>
          ))}
        </m.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
        ...
        >
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-lg font-bold text-[#0F172A] dark:text-slate-100">Cobertura temática</h3>
          </div>
          <p className="text-xs text-slate-500 mb-5">% de municipios con al menos un análisis publicado por tema</p>
        ...
        >
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-lg font-bold text-[#0F172A] dark:text-slate-100">Distribución geográfica</h3>
          </div>
          <p className="text-xs text-slate-500 mb-6">Partidos analizados por región</p>
              ...
              {COBERTURA.map(r => (
                <div key={r.region} className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-600 dark:text-slate-500">{r.region}</span>
                  </div>
                  <span className="text-sm font-semibold text-[#0F172A] dark:text-slate-100">{r.partidos} partidos</span>
                </div>
              ))}
              <p className="text-xs text-slate-500 mt-2">Total: {total} partidos · Fuente: IGN 2024</p>
            </div>
          </div>
        </m.div>
      </div>
      ...
      <m.div
        ...
        viewport={{ once: true }}
        transition={{ duration: 0.5 }}
        className="bg-[#0F172A] rounded-2xl p-8 sm:p-12 text-white"
      >
...
function DonutChart({ data, total }) {
  ...
  return (
    <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} className="shrink-0">
      ...
      <text x={cx} y={cy - 6} textAnchor="middle" fontSize="18" fontWeight="700" className="fill-[#0F172A] dark:fill-slate-100">{total}</text>
