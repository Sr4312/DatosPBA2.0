    ...
    >
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        <div className="flex items-center justify-between gap-2">
          ...
          {hilo.plataforma && <span className="text-xs text-slate-500 dark:text-slate-500 shrink-0">{hilo.plataforma}</span>}
        </div>

        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 leading-snug">{hilo.titulo}</h3>

        {hilo.resumen && <p className="text-xs text-slate-500 dark:text-slate-500 line-clamp-3 flex-1">{hilo.resumen}</p>}
