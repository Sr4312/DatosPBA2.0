...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className={`px-4 sm:px-6 flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-14' : 'h-24'}`}>
            <NavLink to="/" className="flex items-center gap-2 no-underline shrink-0">
              ...
              <div className="flex flex-col leading-tight overflow-hidden">
                <span className="text-xl text-[#0F172A] dark:text-slate-100 tracking-tight">Datos<span className="font-bold">PBA</span></span>
                <span className={`text-[10px] text-slate-500 hidden sm:block leading-snug transition-all duration-300 ${scrolled ? 'max-h-0 opacity-0' : 'max-h-10 opacity-100'}`}>Análisis basado en evidencia<br />para la Provincia de Buenos Aires.</span>
              </div>
            </NavLink>
                  ...
                  className={({ isActive }) =>
                    `px-3 py-1.5 text-base font-medium rounded-lg transition-colors no-underline flex items-center gap-1.5 ${
                      isActive
                        ...
                        : 'text-slate-500 dark:text-slate-500 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-700/20 dark:hover:text-brand-300'
                    }`
                  }
                >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3.5 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-500 hover:text-[#0F172A] dark:hover:text-slate-100 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
                aria-label="Buscar"
              >
                <Search className="w-4 h-4" />
                <span className="text-sm hidden sm:inline">Buscar</span>
                <kbd className="hidden lg:inline-flex items-center gap-0.5 text-[10px] font-medium text-slate-500 border border-slate-200 dark:border-slate-700 rounded px-1.5 py-0.5">
                  Ctrl K
                </kbd>
              </button>
              ...
              {/* Mobile hamburger */}
              <button
                className="lg:hidden p-2 text-slate-500 dark:text-slate-500 hover:text-brand-600 transition-colors"
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `px-3 py-2.5 text-sm font-medium rounded-lg transition-colors no-underline ${
                      isActive
                        ? 'bg-brand-100 dark:bg-brand-700/30 text-brand-700 dark:text-brand-300'
                        : 'text-slate-500 dark:text-slate-500 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-slate-800'
                    }`
                  }
                >
                  {l.label}
      ...
      {/* Contact section */}
      <section className="bg-white dark:bg-slate-900 border-t-2 border-[#0F172A] dark:border-slate-700 mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="md:col-span-1">
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-600 uppercase tracking-widest mt-6 font-medium">
          ...
          {/* Nav links */}
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-4">Contenido</p>
            <ul className="space-y-2.5">
              {NAV.map(l => (
                <li key={l.to}>
                  <NavLink
                    to={l.to}
                    className="text-sm text-slate-600 dark:text-slate-500 hover:text-brand-600 dark:hover:text-brand-400 transition-colors no-underline"
                  >
                    {l.label}
                  </NavLink>
                </li>
          ...
          {/* Contact */}
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-4">Contacto</p>
            <ul className="space-y-3">
              <li>
                <a
                  href="mailto:contacto@datospba.com"
                  className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-500 hover:text-brand-600 dark:hover:text-brand-400 transition-colors no-underline"
                >
              ...
              <li>
                <a
                  ...
                  className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-500 hover:text-brand-600 dark:hover:text-brand-400 transition-colors no-underline"
                >
                  <Twitter className="w-4 h-4 shrink-0 text-slate-500" />
                  @datospba
                </a>
              </li>
      ...
      {showTop && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 w-10 h-10 rounded-full bg-[#0F172A] shadow-lg flex items-center justify-center text-white hover:bg-brand-600 hover:shadow-xl transition-all"
